...
- After 10000 iterations, even though, the agent's knowledge is very good and it's learned how to reach the goal, but the final greedy policy is not optimal, the agent still isn't sure about how good the actions are for some states i.e (1,3). This might be because of the value of epsilon that decides whether to explore the environment or exploit it, and it could also be because of the number of iterations is only 10000.  

- The agent has learned how to reach to the goal from each state in our grid, it takes one path to the goal and it doesn't slip due the $\beta$ value of 0 that doesn't allow the agent to slip from the path.  
